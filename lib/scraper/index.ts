...
export async function scrapeAmazonProduct(url:string){
    ...
    try {
        ...
        const starRatingText = $(".a-icon.a-icon-star-small span.a-icon-alt").first().text().trim();
        const starRating = parseFloat(starRatingText.split(" ")[0]);
        const parsedStarRating = isNaN(starRating) ? 0 : Number(starRating);
        ...
        // console.log({title,currentPrice,originalPrice,outOfStock,imageUrls,discountRate,data})
